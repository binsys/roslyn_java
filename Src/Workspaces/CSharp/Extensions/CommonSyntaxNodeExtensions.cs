    ...
    internal static class CommonSyntaxNodeExtensions
    {
        ...
        public static IEnumerable<UsingDirectiveSyntax> GetEnclosingUsingDirectives(this SyntaxNode node)
        {
            return node.GetAncestorOrThis<CompilationUnitSyntax>().Usings
                       .Concat(node.GetAncestorsOrThis<PackageDeclarationSyntax>()
                                   .Reverse()
        ...
        public static PackageDeclarationSyntax GetInnermostNamespaceDeclarationWithUsings(this SyntaxNode contextNode)
        {
            return contextNode.GetAncestorsOrThis<PackageDeclarationSyntax>().FirstOrDefault(n => n.Usings.Count > 0);
        }
    }
}