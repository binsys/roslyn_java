...
namespace Microsoft.CodeAnalysis.CSharp.Extensions
{
    internal static partial class ExpressionSyntaxExtensions
    {
        ...
        private static int GetNamespaceIdForAliasSearch(SemanticModel semanticModel, SyntaxToken token, CancellationToken cancellationToken)
        {
            ...
            var @namespace = startNode.GetAncestorOrThis<PackageDeclarationSyntax>();
            if (@namespace != null)
            {
        ...
        private static int GetNamespaceId(SyntaxNode container, PackageDeclarationSyntax target, ref int index)
        {
            ...
            var @namespace = container as PackageDeclarationSyntax;
            if (@namespace != null)
            {
        ...
        private static int GetNamespaceId(SyntaxList<MemberDeclarationSyntax> members, PackageDeclarationSyntax target, ref int index)
        {
            foreach (var member in members)
            {
                var childNamespace = member as PackageDeclarationSyntax;
            ...
            while (parent != null)
            {
                switch (parent.CSharpKind())
                {
                    ...
                    case SyntaxKind.NamespaceDeclaration:
                        var namespaceDeclaration = (PackageDeclarationSyntax)parent;
        ...
        private static bool ParserWouldTreatExpressionAsCast(ExpressionSyntax reducedNode, MemberAccessExpressionSyntax originalNode)
        {
            ...
            if (IsNameOrMemberAccessButNoExpression(newExpression))
            {
                ...
                return nextToken.CSharpKind() == SyntaxKind.OpenParenToken ||
                    ...
                    (SyntaxFacts.IsKeywordKind(nextToken.CSharpKind()) && !(nextToken.CSharpKind() == SyntaxKind.AsKeyword || nextToken.CSharpKind() == SyntaxKind.InstanceOfKeyword));
            }
