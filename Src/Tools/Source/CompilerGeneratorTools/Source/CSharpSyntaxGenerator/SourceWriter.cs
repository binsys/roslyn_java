...
namespace CSharpSyntaxGenerator
{
	class SourceWriter : AbstractFileWriter
	{
		...
		private void WriteFile()
		{
			...
			WriteLine("#region Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax");
			...
			WriteLine("#region Microsoft.CodeAnalysis.CSharp.Syntax");
			WriteLine();
			...
			WriteLine("#region Microsoft.CodeAnalysis.CSharp");
			WriteLine();
		...
		{
			WriteLine("\t#region GreenVisitors");
			//WriteGreenVisitor(true, true);
			//WriteLine();

			WriteLine("\t#region GreenVisitors withresult");
			WriteGreenVisitor(false, true);
			WriteLine();
			WriteLine("\t#endregion");

			WriteLine("\t#region GreenVisitors withoutresult");
		...
		private void WriteGreenRewriter()
		{
			...
			WriteLine();
			WriteLine("\t#region GreenRewriter");
		...
		private void WriteContextualGreenFactories()
		{
			...
			WriteLine();
			WriteLine("\t#region ContextualGreenFactories");
		...
		private void WriteStaticGreenFactories()
		{
			var nodes = Tree.Types.Where(n => !(n is PredefinedNode) && !(n is AbstractNode)).ToList();
			WriteLine();
			WriteLine("\t#region StaticGreenFactories");
		...
		{
			this.WriteLine("\t#region RedTypes");
			var nodes = Tree.Types.Where(n => !(n is PredefinedNode)).ToList();
			for (int i = 0, n = nodes.Count; i < n; i++)
			{
				var node = nodes[i];
		...
		{
			this.WriteLine("\t#region {0}", node.Name);
			WriteComment(node.TypeComment, "\t");
		...
		{
			WriteLine("\t\t#region RedAcceptMethods");
		...
		{
			WriteLine("#region WriteRedVisitors");
