...
namespace Microsoft.CodeAnalysis.CSharp.Syntax
{
	internal class SyntaxFormatter : CSharpSyntaxRewriter
	{
		...
		private int LineBreaksAfter(SyntaxToken currentToken, SyntaxToken nextToken)
		{
			...
			switch (currentToken.CSharpKind())
			{
				...
				case SyntaxKind.CloseBracketToken:
					if (currentToken.Parent is AnnotationSyntax)
					{
						return 1;
					}
			...
			switch (nextToken.CSharpKind())
			{
				...
				case SyntaxKind.OpenBracketToken:
					return nextToken.Parent is AnnotationSyntax ? 1 : 0;
			}
		...
		private bool NeedsSeparator(SyntaxToken token, SyntaxToken next)
		{
			...
			{
				if (!SyntaxKindFacts.IsPunctuation(next.CSharpKind()))
				{
					return true;
				}
			}
			...
			{
				if (next.Parent.IsKind(SyntaxKind.ImplementsListClause) ||
					next.Parent.IsKind(SyntaxKind.TypeBound))
				{
					return true;
				}
			}
			...
			if (SyntaxKindFacts.IsLiteral(token.CSharpKind()) && SyntaxKindFacts.IsLiteral(next.CSharpKind()))
			{
				return true;
			}
		...
		{
			return SyntaxKindFacts.IsKeywordKind(kind);
		}
