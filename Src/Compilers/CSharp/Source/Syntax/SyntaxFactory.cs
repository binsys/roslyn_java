...
namespace Microsoft.CodeAnalysis.CSharp
{
	...
	public static partial class SyntaxFactory
	{
		...
		/// <summary>
		/// Parse an AnnotationArgumentListSyntax node.
		/// </summary>
		/// <param name="text">The text of the _annotation argument list.</param>
		/// <param name="offset">Optional offset into text.</param>
		/// <param name="options">The optional parse options to use. If no options are specified default options are
		/// used.</param>
		/// <param name="consumeFullText">True if extra tokens in the input should be treated as an error</param>
		public static AnnotationArgumentListSyntax ParseAttributeArgumentList(string text, int offset = 0, ParseOptions options = null, bool consumeFullText = true)
		{
			using (var lexer = MakeLexer(text, offset, (CSharpParseOptions)options))
			using (var parser = MakeParser(lexer))
			{
				var node = parser.ParseAnnotationArgumentList();
				if (consumeFullText) node = parser.ConsumeUnexpectedTokens(node);
				return (AnnotationArgumentListSyntax)node.CreateRed();
			}
		}
